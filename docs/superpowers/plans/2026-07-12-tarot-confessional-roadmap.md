...
- [ ] 建立 78 张 `deck.json`，固定稳定 ID。
- [ ] 编写 `tarot_codec.py`，支持 F1/S3/R3。
- [ ] 在 JavaScript 和 Python 中实现同一协议。
- [ ] 建立正常、逆位、边界值和损坏码测试向量。
- [ ] 编写 `decode_draw.py` CLI 和结构化 JSON 输出。
